...
namespace EVMESCharts.ModBus
{
    class ModBusHelp
    {
        ...
        public static void ReadModBus(int ID = 0)
        {
            ...
            //------------------------------------------------------------------------------------------------------------------------
            // 读取每个小时的稼动率数据
            List<int> ProduceList = ModBusHelp.HoldingRegister(200, 24);
            //------------------------------------------------------------------------------------------------------------------------
            // 读取每小时的电气消耗数据
            // 电消耗数据
            List<int> PowerList = ModBusHelp.HoldingRegister(230, 24);
            // 气消耗数据
            List<int> GasList = ModBusHelp.HoldingRegister(260, 24);
            ...
            for (int i = 0; i <= Hour; i++)
            {
                ...
                {
                    //更新数据库数据
                    string hourupdatasql = $"UPDATE DayDeviceData SET Reach = {HourReachList[i]},Produce = {HourProduceList[i]},TimeEfficiency = {ProduceList[i]} WHERE Day = '{NowDay}' AND Time = {i} AND DevID = {ID}";
                    SQLiteHelp.SQLUpdate(hourupdatasql);
                }
                else
                {
                    // 插入数据
                    string hourinsertsql = $"INSERT INTO DayDeviceData VALUES ('{NowDay}',{i},{ID},{HourProduceList[i]},{HourReachList[i]},{ProduceList[i]},{0},{0},{0})";
                ...
                {
                    //更新数据库数据
                    string hourupdatasql = $"UPDATE DayDeviceData SET TimeEfficiency = {ProduceList[i]} WHERE Day = '{NowDay}' AND Time = {i} AND DevID = {ID}";
                    SQLiteHelp.SQLUpdate(hourupdatasql);
            ...
            {
                // 更新数据库
                string updatasql = $"UPDATE MonthDeviceData SET TimeEfficiency = {ProduceList[0]},Produce = {ToDaySum} WHERE Day = '{NowDay}' AND DevID = {ID}";
                SQLiteHelp.SQLUpdate(updatasql);
            }
            else
            {
                // 插入数据
                string insertsql = $"INSERT INTO MonthDeviceData VALUES ('{NowDay}',{ID},{ToDaySum},{ProduceList[0]},{0},{0},{0})";
                ...
                if (SelData.Rows.Count > 0)
                {
                    ...
                    if (YesData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < TotalWorkStation.Count; i++)
                        {
                            ...
                            // 更新数据库
                            string upsql = $"UPDATE DetailedFault SET Number = {nowvalue},Duration = {0} WHERE ID = {i} AND TimeSlot ={0} AND Day = '{NowDay}'";
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < TotalWorkStation.Count; i++)
                        {
                            string upsql = $"UPDATE DetailedFault SET Number = {TotalWorkStation[i]},Duration = {0} WHERE ID = {i} AND TimeSlot ={0} AND Day = '{NowDay}'";
                            SQLiteHelp.SQLUpdate(upsql);
                        }
                    }
                }
                else
                {
                    ...
                    if (YesData.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < TotalWorkStation.Count; i++)
                        {
                            ...
                            // 插入数据库
                            string insesql = $"INSERT INTO DetailedFault VALUES ('{NowDay}',{0},{i},{nowvalue},{0})";
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < TotalWorkStation.Count; i++)
                        {
                            string insesql = $"INSERT INTO DetailedFault VALUES ('{NowDay}',{0},{i},{TotalWorkStation[i]},{0})";
                            SQLiteHelp.SQLInsert(insesql);
                        }
                    } 
                }
